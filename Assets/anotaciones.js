...
const writeMonth = (month) => {/*Función encargada de escribir los meses, recibe como parametros mes*/
    ...
    for(let i=1; i<=getTotalDays(month); i++){/*getTotalDays dice el total de día del mes*/
        if(i===currentDay) { /*Comprovamos si el día es hoy, si el día es hoy se le agrega la clase today*/
            dates.innerHTML += ` <div class="calendar__date calendar__item calendar__today">${i}</div>`; /*se crea un div con las clases entre comillas con el contenido que valga i*/
        }else{
